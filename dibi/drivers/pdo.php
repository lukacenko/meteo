	...
	public function applyLimit(&$sql, $limit, $offset)
	{
		...
		case 'sqlsrv':
			if ($offset < 1) {
			    ...
			    $polia = explode(',', $sql);
			    $order_key = array_values(preg_grep('/order by/i', $polia));
			    $order = '';
			    $replace = false;
			    $select = '';
			    if(!empty($order_key)){
				$order_key = explode('||||', str_ireplace('order by', '||||', $order_key[0]));
				$order = explode(' ', trim(end($order_key)));
				if(isset($order[1])){
				...
				$id_key = array_values(preg_grep('/ as id/i', $polia));
				if(!empty($id_key)){
				    $order = str_replace('SELECT', '', $id_key[0]);
				    $order = ' '.trim(str_replace('as id', '', $order));
				}else{
			    ...
			    if($replace == true){
				$select = str_replace('ORDER BY'.$order, '', $sql);
				$select = substr_replace($select, ', ROW_NUMBER() OVER (ORDER BY'. $order.') AS RowNum', stripos($select, ' from '), 0);
			    }
			    else{
				$select = substr_replace($sql, ', ROW_NUMBER() OVER (ORDER BY'. $order.') AS RowNum', stripos($sql, ' from '), 0);
			    }
			    $start = (string)$offset;
			    $end = (string)((int)$offset + (int)$limit);
